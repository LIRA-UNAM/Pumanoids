...
// Return the elapsed milliseconds since start_time.
...
inline std::string gen_uuid() {
    ...
    ss << "-4"; // Digit 13 is 4, indicating a version 4 UUID.
    for (int i = 0; i < 3; i++) ss << uni(rng);
    ss << "-";
    ss << uni8(rng); // Digit 17 is 8, 9, A, or B.
    for (int i = 0; i < 3; i++) ss << uni(rng);
    ss << "-";
...
inline std::string gen_timestamp_str() {
    ...
    now += std::chrono::hours(8); // Force UTC+8.
