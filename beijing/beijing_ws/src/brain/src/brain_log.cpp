...
BrainLog::BrainLog(Brain *argBrain) : brain(argBrain), log_tcp("robocup"), log_file("robocup")
{
    ...
    if (enable_log_tcp)
    {
        ...
        // If no service is listening, this may block for the default timeout (2 seconds) without reporting an error.
        // TODO: Check the Rerun documentation and report connection failures correctly.
    }
...
bool BrainLog::shouldLog(std::string_view throttle_key, double max_hz)
{
    if (!enabled())
        return false;
    if (!std::isfinite(max_hz) || max_hz <= 0.0)
        return true;

    const auto now = std::chrono::steady_clock::now();
    const std::chrono::duration<double> min_interval(1.0 / max_hz);
    std::lock_guard<std::mutex> lock(rate_limit_mutex);
    auto [it, inserted] = last_log_times.emplace(std::string(throttle_key), now);
    if (inserted)
        return true;
    if (now - it->second < min_interval)
        return false;
...
{
    if (!shouldLog("screen_visual/" + logPath, brain->config->rerunLogVisualHz))
        return;
