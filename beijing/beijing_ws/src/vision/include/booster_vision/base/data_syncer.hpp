...
struct PoseDataBlock : DataBlock<Pose> {
    PoseDataBlock() = default;
    PoseDataBlock(const Pose &data, double timestamp) :
        DataBlock<Pose>(data, timestamp) {
    }

    // For interpolated poses, timestamp is the requested image time while
    // nearest_source_diff still measures freshness against a real sample.
    double nearest_source_diff{0.0};
...
struct SyncedDataBlock {
    ...
    SyncedDataBlock(const SyncedDataBlock &other) :
        pose_data(other.pose_data), pose_valid(other.pose_valid) {
...
class DataSyncer {
public:
    explicit DataSyncer(bool enable_depth,
                        double pose_interpolation_max_gap_ms = 0.0,
                        double pose_wait_timeout_ms = 0.0,
                        double depth_wait_timeout_ms = 0.0) :
        enable_depth_(enable_depth),
        pose_interpolation_max_gap_sec_(
            std::max(0.0, pose_interpolation_max_gap_ms) * 1e-3),
        pose_wait_timeout_ms_(std::max(0.0, pose_wait_timeout_ms)),
        depth_wait_timeout_ms_(std::max(0.0, depth_wait_timeout_ms)) {
    }
...
private:
    bool enable_depth_;
    double pose_interpolation_max_gap_sec_{0.0};
    double pose_wait_timeout_ms_{0.0};
    double depth_wait_timeout_ms_{0.0};

    std::mutex depth_buffer_mutex_;
    std::condition_variable depth_buffer_cv_;
    std::mutex pose_buffer_mutex_;
    std::condition_variable pose_buffer_cv_;
    ...
    std::vector<PoseDataBlock> pose_time_reset_candidates_;
};
