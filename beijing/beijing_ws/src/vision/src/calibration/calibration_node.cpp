            ...
            if (key_sys == 'y') {
                ...
                if (!sys_cfg) {
                    ...
                    std::string tmp_path = "/tmp/vision.yaml";
                    {
                        ...
                        if (tmp_out) {
                            tmp_out << oss.str();
                            std::cout << "[fallback] Wrote temporary file: " << tmp_path << std::endl;
                        } else {
                            std::cerr << "[fallback] Failed to write temporary file: " << tmp_path << std::endl;
                        }
                    }
                } else {
...
void CalibrationNode::PoseCallback(
    ...
    if (stamp_ns <= 0 || !std::isfinite(quaternion_norm_sq) ||
        quaternion_norm_sq < 1e-12 || !std::isfinite(position.x) ||
        !std::isfinite(position.y) || !std::isfinite(position.z) ||
        !std::isfinite(orientation.x) || !std::isfinite(orientation.y) ||
        !std::isfinite(orientation.z) || !std::isfinite(orientation.w)) {
        RCLCPP_WARN_THROTTLE(
            get_logger(), *get_clock(), 5000,
            "Ignoring invalid head pose sample");
        return;
    }

    const double inv_norm = 1.0 / std::sqrt(quaternion_norm_sq);
    const Pose pose(
        static_cast<float>(position.x),
        static_cast<float>(position.y),
        static_cast<float>(position.z),
        static_cast<float>(orientation.x * inv_norm),
        static_cast<float>(orientation.y * inv_norm),
        static_cast<float>(orientation.z * inv_norm),
        static_cast<float>(orientation.w * inv_norm));
    const double timestamp = static_cast<double>(stamp_ns) * 1e-9;
    data_syncer_->AddPose(PoseDataBlock(pose, timestamp));
}

void CalibrationNode::CameraInfoCallback(
    const sensor_msgs::msg::CameraInfo::ConstSharedPtr msg) {
    if (!msg) {
