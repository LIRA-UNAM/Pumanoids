...
static std::size_t img_buffer_capacity = 0;
...
void cuda_preprocess(uint8_t* src, int src_width, int src_height, std::size_t src_step, float* dst,
                     int dst_width, int dst_height, cudaStream_t stream) {
    const std::size_t packed_step = static_cast<std::size_t>(src_width) * 3;
    const std::size_t img_size = packed_step * static_cast<std::size_t>(src_height);
    if (img_size > img_buffer_capacity) {
        cuda_preprocess_init(src_width * src_height);
    }

    if (src_step == packed_step) {
        std::memcpy(img_buffer_host, src, img_size);
    } else {
        for (int row = 0; row < src_height; ++row) {
            std::memcpy(
                img_buffer_host + static_cast<std::size_t>(row) * packed_step,
                src + static_cast<std::size_t>(row) * src_step, packed_step);
        }
    }
    ...
    warpaffine_kernel<<<blocks, threads, 0, stream>>>(img_buffer_device, src_width * 3, src_width, src_height, dst,
                                                      dst_width, dst_height, 114, d2s, jobs);
}
...
void cuda_batch_preprocess(std::vector<cv::Mat>& img_batch, float* dst, int dst_width, int dst_height,
    ...
    for (size_t i = 0; i < img_batch.size(); i++) {
        cuda_preprocess(img_batch[i].ptr(), img_batch[i].cols, img_batch[i].rows,
                        img_batch[i].step, &dst[dst_size * i], dst_width, dst_height, stream);
...
void cuda_preprocess_init(int max_image_size) {
    const std::size_t required_capacity =
        static_cast<std::size_t>(max_image_size) * 3;
    if (img_buffer_host != nullptr && img_buffer_device != nullptr &&
        required_capacity <= img_buffer_capacity) {
        return;
    }
    ...
    CUDA_CHECK(cudaMallocHost((void**)&img_buffer_host, required_capacity));
    // prepare input data in device memory
    CUDA_CHECK(cudaMalloc((void**)&img_buffer_device, required_capacity));
    img_buffer_capacity = required_capacity;
}
