...
std::shared_ptr<YoloV8Detector> YoloV8Detector::CreateYoloV8Detector(const YAML::Node &node, const std::string model_path_override) {
    try {        
        ...
        std::string preprocess_backend = node["preprocess_backend"]
            ? node["preprocess_backend"].as<std::string>() : "cpu";
        std::string decode_backend = node["decode_backend"]
            ? node["decode_backend"].as<std::string>() : "cpu";
        ...
        if (detector_ptr) {
            ...
            detector_ptr->setPreprocessBackend(preprocess_backend);
            detector_ptr->setDecodeBackend(decode_backend);
            if (model_path.find(".engine") != std::string::npos) {
                // Backend selection must be set before TensorRT allocates buffers.
                detector_ptr->Init(model_path);
            }
        }
        return detector_ptr;
