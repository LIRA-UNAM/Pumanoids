...
float BallRadiusFromSize(int ball_size) {
    switch (ball_size) {
        case 3: return 0.09f;
        case 4: return 0.095f;
        case 5: return 0.105f;
        default:
            throw std::invalid_argument("ball_size must be 3, 4, or 5");
    }
}

cv::Point3f CalculatePositionByIntersection(const Pose &p_eye2base, const cv::Point2f target_uv,
                                             const Intrinsics &intr) {
    return CalculatePositionByIntersection(p_eye2base, target_uv, intr, 0.0f);
}

cv::Point3f CalculatePositionByIntersection(const Pose &p_eye2base, const cv::Point2f target_uv,
                                             const Intrinsics &intr, float ground_height) {
    ...
    float scale = (ground_height - mat_trans.at<float>(2, 0)) /
        mat_rot_obj_ray.at<float>(2, 0);
...
void BallPoseEstimator::Init(const YAML::Node &node) {
    ...
    const int ball_size = as_or<int>(node["ball_size"], 3);
    radius_ = BallRadiusFromSize(ball_size);
    ...
    std::cout << "ball_size: " << ball_size << ", radius: " << radius_ << "m" << std::endl;
...
Pose BallPoseEstimator::EstimateByColor(const Pose &p_eye2base, const DetectionRes &detection, const cv::Mat &rgb) {
    ...
    const cv::Point2f target_uv(
        bbox.x + bbox.width / 2.0f, bbox.y + bbox.height / 2.0f);
    const cv::Point3f target_xyz =
        CalculatePositionByIntersection(p_eye2base, target_uv, intr_, radius_);
    return Pose(target_xyz.x, target_xyz.y, target_xyz.z, 0, 0, 0);
}
