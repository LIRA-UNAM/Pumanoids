...
void CreatePointCloud(pcl::PointCloud<pcl::PointXYZRGB>::Ptr &cloud, const cv::Mat &depth_image, const cv::Mat &rgb_image, const cv::Rect &bbox,
    ...
    if (!cloud || depth_image.empty() || depth_image.type() != CV_32FC1) return;

    const bool hasRgb = !rgb_image.empty() && rgb_image.type() == CV_8UC3;
    // Detection boxes are in RGB coordinates.  T2 currently publishes an
    // aligned stream, but mapping the box also keeps this safe for resized
    // depth frames.
    const double sx = hasRgb && rgb_image.cols > 0
        ? static_cast<double>(depth_image.cols) / rgb_image.cols : 1.0;
    const double sy = hasRgb && rgb_image.rows > 0
        ? static_cast<double>(depth_image.rows) / rgb_image.rows : 1.0;
    cv::Rect depthBbox(
        static_cast<int>(std::floor(bbox.x * sx)),
        static_cast<int>(std::floor(bbox.y * sy)),
        static_cast<int>(std::ceil(bbox.width * sx)),
        static_cast<int>(std::ceil(bbox.height * sy)));
    depthBbox &= cv::Rect(0, 0, depth_image.cols, depth_image.rows);
    if (depthBbox.empty()) return;

    Intrinsics depthIntrinsics = intrinsics;
    if (hasRgb && (depth_image.cols != rgb_image.cols || depth_image.rows != rgb_image.rows)) {
        depthIntrinsics.fx *= static_cast<float>(sx);
        depthIntrinsics.cx *= static_cast<float>(sx);
        depthIntrinsics.fy *= static_cast<float>(sy);
        depthIntrinsics.cy *= static_cast<float>(sy);
    }

    for (int v = depthBbox.y; v < depthBbox.y + depthBbox.height; ++v) {
        for (int u = depthBbox.x; u < depthBbox.x + depthBbox.width; ++u) {
            float depth = depth_image.at<float>(v, u);
            if (std::isfinite(depth) && depth > 0) {
                auto cv_point = depthIntrinsics.BackProject(cv::Point2f(u, v), depth);
                ...
                point.z = cv_point.z;
                if (hasRgb) {
                    const int rgbU = std::clamp(
                        static_cast<int>(std::lround((u + 0.5) / sx - 0.5)),
                        0, rgb_image.cols - 1);
                    const int rgbV = std::clamp(
                        static_cast<int>(std::lround((v + 0.5) / sy - 0.5)),
                        0, rgb_image.rows - 1);
                    const auto color = rgb_image.at<cv::Vec3b>(rgbV, rgbU);
                    point.b = color[0];
                    point.g = color[1];
                    point.r = color[2];
                } else {
