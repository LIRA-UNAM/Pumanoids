...
enum class State {idle, ball_align, forward, goal_align}; 

constexpr std::string_view to_string(State state) {
    switch (state) {
        case State::idle:       return "idle";
        case State::ball_align: return "ball_align";
        case State::forward:    return "forward";
        case State::goal_align: return "goal_align";
...
class GoToTarget : public rclcpp::Node
{
    ...
    private:
        ...
        {
            RCLCPP_DEBUG(this->get_logger(), "Target point x= '%.3f' y= '%.3f' a= '%0.3f'", msg->x, msg->y, msg->theta);
            success.data = false;
        ...
        void timer_callback()
        {
            ...
            {
                RCLCPP_DEBUG(this->get_logger(), "Waiting for pumas_map -> pumas_base_link");
            }
